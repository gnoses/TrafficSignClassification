function [net, info] = cnn_trafficsign(varargin)
...
opts.train.numEpochs = 3000;
opts.train.continue = false;
opts.train.useGpu = false;
opts.train.learningRate = [0.05*ones(1, 20) 0.01*ones(1,30) 0.001] ;
...
net.layers{end+1} = struct('type', 'conv', ...
                           'filters', f*randn(3,3,1,50, 'single'), ...
...
net.layers{end+1} = struct('type', 'conv', ...
                           'filters', f*randn(3,3,50,50, 'single'),...
                           'biases', zeros(1,50,'single'), ...
                           'stride', 1, ...
                           'pad', 0) ;
net.layers{end+1} = struct('type', 'relu') ;
...
net.layers{end+1} = struct('type', 'conv', ...
                           'filters', f*randn(3,3,50,100, 'single'),...
...
net.layers{end+1} = struct('type', 'conv', ...
                           'filters', f*randn(4,4,100,400, 'single'),...
                           'biases', zeros(1,400,'single'), ...
                           'stride', 1, ...
                           'pad', 0) ;
                   
net.layers{end+1} = struct('type', 'conv', ...
                           'filters', f*randn(1,1,400,62, 'single'),...
                           'biases', zeros(1,62,'single'), ...
                           'stride', 1, ...
                           'pad', 0) ;
